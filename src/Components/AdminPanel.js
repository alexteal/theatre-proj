...
const AdminPanel = ({ currentlyRunning, setCurrentlyRunning, comingSoon, setComingSoon }) => {
  ...
  const handleUpdateMovie = () => {
    const updatedCurrentlyRunning = currentlyRunning.map((movie) =>
      movie === selectedMovie ? { ...movie, title: editedTitle, trailerUrl: editedTrailerUrl } : movie
    );
  ...
  const handleDeleteMovie = (movie, category) => {
    if (category === 'currentlyRunning') {
      const updatedCurrentlyRunning = currentlyRunning.filter((item) => item !== movie);
      setCurrentlyRunning(updatedCurrentlyRunning);
    } else if (category === 'comingSoon') {
      const updatedComingSoon = comingSoon.filter((item) => item !== movie);
      setComingSoon(updatedComingSoon);
    }
          ...
          <>
            ...
            <div>
              ...
              {currentlyRunning.map((movie, index) => (
                <div key={index}>
                  <button onClick={() => handleSelectMovie(movie, 'currentlyRunning')}>{movie.title}</button>
                  <button onClick={() => handleDeleteMovie(movie, 'currentlyRunning')}>Delete</button>
                </div>
              ))}
            </div>
            <div>
              ...
              {comingSoon.map((movie, index) => (
                <div key={index}>
                  <button onClick={() => handleSelectMovie(movie, 'comingSoon')}>{movie.title}</button>
                  <button onClick={() => handleDeleteMovie(movie, 'comingSoon')}>Delete</button>
                </div>
              ))}
            </div>
            {selectedMovie && (
              <div>
                ...
                <input value={editedTrailerUrl} onChange={(e) => setEditedTrailerUrl(e.target.value)} placeholder="Trailer URL" />
            ...
            <div className='mt-5'>
              ...
              />
              <button onClick={() => handleAddMovie('currentlyRunning')}>Add to Currently Running</button>
              <button onClick={() => handleAddMovie('comingSoon')}>Add to Coming Soon</button>
            </div>
          </>
        );
      default:
  ...
  return (
    <div className="admin-panel">
      <nav className="admin-nav">
        <button onClick={() => setActiveTab('manageMovies')}>Manage Movies</button>
